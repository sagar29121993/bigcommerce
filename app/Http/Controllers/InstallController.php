...
use GuzzleHttp\RequestOptions as Guzzlerequest;
...
class InstallController extends BaseController
{
    ...
    public function install_page_submit(Request $request)
    {
    // Make sure all required query params have been passed
        if (!$request->has('code') || !$request->has('scope') || !$request->has('context')) {
            return redirect()->route('error')->with('error_message', 'Not enough information was passed to install this app.');
        }
        ...
        try {
            ...
            if ($request->has('external_install')) {
                return redirect()->route('install_page');
            } else {
                return redirect()->route('error')->with('error_message', $errorMessage);
            }
        }
    }
    ...
    public function load(Request $request)
    {
        ...
        if (!empty($signedPayload)) {
            ...
            if ($verifiedSignedRequestData !== null) {
                ...
                return redirect()->route('error')->with('error_message', 'The signed request from BigCommerce could not be validated.');
            }
        } else {
            return redirect()->route('error')->with('error_message', 'The signed request from BigCommerce was empty.');
        }

        return redirect('/');
    }

    public function installled(){
        return "Hellow World";
    }
